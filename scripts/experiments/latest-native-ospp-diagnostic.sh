...
# from the older known-good Office install because the latest x86 install does
# not contain ProgramFilesCommonX86 OSPP files.
#
# Usage:
#   latest-native-ospp-diagnostic.sh apply
#   latest-native-ospp-diagnostic.sh status
#   latest-native-ospp-diagnostic.sh restore
...
case "$MODE" in
  ...
  restore)
    if [[ -f "$PREFIX/drive_c/windows/system32/sppc.dll.before-osppc-test" ]]; then
      cp "$PREFIX/drive_c/windows/system32/sppc.dll.before-osppc-test" "$PREFIX/drive_c/windows/system32/sppc.dll"
    fi
    if [[ -f "$PREFIX/drive_c/windows/syswow64/sppc.dll.before-osppc-test" ]]; then
      cp "$PREFIX/drive_c/windows/syswow64/sppc.dll.before-osppc-test" "$PREFIX/drive_c/windows/syswow64/sppc.dll"
    fi
    wine_reg add 'HKCU\Software\Wine\DllOverrides' /v sppc /t REG_SZ /d builtin /f
    ;;
  status)
    echo "PREFIX=$PREFIX"
    echo "Latest x64 OSPP source:"
    find "$latest_ospp_x64" -maxdepth 1 -type f -printf '  %f\n' | sort
    echo "Current OSPP/SPP files:"
    for file in \
      "$target_ospp_x64/OSPPC.DLL" \
      "$PREFIX/drive_c/windows/system32/sppc.dll" \
      "$PREFIX/drive_c/windows/syswow64/sppc.dll"
    do
